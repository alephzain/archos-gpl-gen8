...
GURI *gnet_uri_new( const char *uri )
{
	...
	ng_return_val_if_fail( uri, NULL );
	...
	while ( *p && *p != ':' && *p != '/' && *p != '?' && *p != '#' )
		++p;
	if ( *p == ':' ) {
		guri->scheme = ng_strndup( temp, p - temp );
		++p;
	...
	if ( *p == '/' && p[1] == '/' ) {
		...
		while ( *p && *p != '@' && *p != '/' )	/* Look for @ or / */
			++p;
		if ( *p == '@' ) {	/* Found userinfo */
			userinfo = ng_strndup( temp, p - temp );
		...
		if ( *p == '[' ) {
			...
			if ( ( p - temp ) == 0 )
				goto error;
			guri->hostname = ng_strndup( temp, p - temp );
			if ( *p )
				p++;	/* Skip ] (if there) */
		} else {
			temp = p;
...
// skip leading "/"
	...
	while ( *p /*&& *p != '?' && *p != '#' */  )
		++p;
	if ( p != temp )
		guri->path = ng_strndup( temp, p - temp );
	else
		guri->path = ng_strndup( "", 0 );
	...
	if ( *p == '?' ) {
		...
		while ( *p && *p != '#' )
			++p;
		guri->query = ng_strndup( temp, p - temp );
	}

	/* Fragment */
	if ( *p == '#' ) {
		++p;
		guri->fragment = ng_strdup( p );
	}
 ...
 *
 **/
GURI *gnet_uri_new_fields( const char *scheme, const char *hostname, const ngint port, const char *path )
{
	GURI *uri = NULL;

	uri = ng_new0( GURI, 1 );
	if ( scheme )
		uri->scheme = ng_strdup( scheme );
	if ( hostname )
		uri->hostname = ng_strdup( hostname );
	uri->port = port;
	if ( path )
		uri->path = ng_strdup( path );
 ...
 *
 **/
GURI *gnet_uri_new_fields_all( const char *scheme, const char *user, const char *passwd, const char *hostname, const ngint port, const char *path, const char *query, const char *fragment )
{
	GURI *uri = NULL;
	...
	if ( scheme )
		uri->scheme = ng_strdup( scheme );
	if ( user )
		uri->user = ng_strdup( user );
	if ( passwd )
		uri->passwd = ng_strdup( passwd );
	if ( hostname )
		uri->hostname = ng_strdup( hostname );
	uri->port = port;
	if ( path )
		uri->path = ng_strdup( path );
	if ( query )
		uri->query = ng_strdup( query );
	if ( fragment )
		uri->fragment = ng_strdup( fragment );
...
GURI *gnet_uri_clone( const GURI * uri )
{
	...
	uri2 = ng_new0( GURI, 1 );
	uri2->scheme = ng_strdup( uri->scheme );
	uri2->user = ng_strdup( uri->user );
	uri2->passwd = ng_strdup( uri->passwd );
	uri2->hostname = ng_strdup( uri->hostname );
	uri2->port = uri->port;
	uri2->path = ng_strdup( uri->path );
	uri2->query = ng_strdup( uri->query );
	uri2->fragment = ng_strdup( uri->fragment );
...
void gnet_uri_delete( GURI * uri )
{
	if ( uri ) {
		...
		ng_free( uri->user );
		ng_free( uri->passwd );
		ng_free( uri->hostname );
		ng_free( uri->path );
		ng_free( uri->query );
		ng_free( uri->fragment );
		ng_free( uri );
	}
}
 ...
 *
 **/
int gnet_uri_equal( ngconstpointer p1, ngconstpointer p2 )
{
	const GURI *uri1 = ( const GURI * ) p1;
	const GURI *uri2 = ( const GURI * ) p2;

	ng_return_val_if_fail( uri1, FALSE );
	ng_return_val_if_fail( uri2, FALSE );

	if ( uri1->port == uri2->port &&
	     !SAFESTRCMP( uri1->scheme, uri2->scheme ) &&
	     !SAFESTRCMP( uri1->user, uri2->user ) &&
	     !SAFESTRCMP( uri1->passwd, uri2->passwd ) &&
	     !SAFESTRCMP( uri1->hostname, uri2->hostname ) && !SAFESTRCMP( uri1->path, uri2->path ) && !SAFESTRCMP( uri1->query, uri2->query ) && !SAFESTRCMP( uri1->fragment, uri2->fragment ) )
		return TRUE;
...
/*
nguint
gnet_uri_hash (ngconstpointer p)
{
  const GURI* uri = (const GURI*) p;
  nguint h = 0;

  ng_return_val_if_fail (uri, 0);

  if (uri->scheme)	h =  ng_str_hash (uri->scheme);
  if (uri->user)	h ^= ng_str_hash (uri->user);
  if (uri->passwd)	h ^= ng_str_hash (uri->passwd);
  if (uri->hostname)	h ^= ng_str_hash (uri->hostname);
  h ^= uri->port;
  if (uri->path)	h ^= ng_str_hash (uri->path);
  if (uri->query)	h ^= ng_str_hash (uri->query);
  if (uri->fragment)	h ^= ng_str_hash (uri->fragment);
...
static char *field_escape( char *str, unsigned char mask )
{
	...
	len = 0;
	for ( i = 0; str[i]; i++ ) {
		if ( neednt_escape_table[( nguint ) str[i]] & mask )
			len++;
		else {
			len += 3;
			must_escape = TRUE;
...
static void field_unescape( char *s )
{
	char *src;
	char *dst;
...
{
	char *rv = NULL;
	NGString *buffer = NULL;

	ng_return_val_if_fail( uri, NULL );

	buffer = ng_string_sized_new( 16 );

	if ( uri->scheme )
		ng_string_sprintfa( buffer, "%s:", uri->scheme );

	if ( uri->user || uri->passwd || uri->hostname || uri->port )
		ng_string_append( buffer, "//" );

	if ( uri->user ) {
		buffer = ng_string_append( buffer, uri->user );
		buffer = ng_string_append_c( buffer, '@' );
	}
	if ( uri->passwd ) {
		buffer = ng_string_append( buffer, uri->passwd );
		buffer = ng_string_append_c( buffer, '@' );
	}
	...
	if ( uri->hostname ) {
		if ( strchr( uri->hostname, ':' ) == NULL )
			buffer = ng_string_append( buffer, uri->hostname );
		else
			ng_string_sprintfa( buffer, "[%s]", uri->hostname );
	}

	if ( uri->port )
		ng_string_sprintfa( buffer, ":%d", uri->port );

	if ( uri->path ) {
		if ( *uri->path == '/' || !( uri->user || uri->passwd || uri->hostname || uri->port ) )
			ng_string_append( buffer, uri->path );
		else
			ng_string_sprintfa( buffer, "/%s", uri->path );
	}

	if ( uri->query )
		ng_string_sprintfa( buffer, "?%s", uri->query );

	if ( uri->fragment )
		ng_string_sprintfa( buffer, "#%s", uri->fragment );

	/* Free only GString not data contained, return the data instead */
	rv = buffer->str;
	ng_string_free( buffer, FALSE );
	return rv;
}
 ...
 *
 **/
void gnet_uri_set_port( GURI * uri, ngint port )
{
	uri->port = port;
}
