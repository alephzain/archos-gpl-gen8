 ...
 *       Optimized to convert flv stream to mpeg4 at run time.
...
#if def C_OPT_MG
	M4V_BLOCK * blk = &mb->block[0];

	if (blk++->last_index >= 0)	cbp |= 1 << 5;
	if (blk++->last_index >= 0)	cbp |= 1 << 4;
	if (blk++->last_index >= 0)	cbp |= 1 << 3;
	if (blk++->last_index >= 0)	cbp |= 1 << 2;
	if (blk++->last_index >= 0)	cbp |= 1 << 1;
	if (blk->last_index >= 0)	cbp |= 1 << 0;
...
static void __inline encode_mb_intra_internal(BW* bw, M4V_MICROBLOCK* mb, int iframe)
{
	...
	if (blk++->last_index >= 1)	cbp |= 1 << 5;
	if (blk++->last_index >= 1)	cbp |= 1 << 4;
	if (blk++->last_index >= 1)	cbp |= 1 << 3;
	if (blk++->last_index >= 1)	cbp |= 1 << 2;
	if (blk++->last_index >= 1)	cbp |= 1 << 1;
	if (blk->last_index >= 1)	cbp |= 1 << 0;
#else
	for (i = 0; i < 6; i++)
	{
	...
	put_zero_bits(bw, 1); // AC Prediction = no
...
static int __inline encode_vo_header(BW* p)
{
	...
	put_zero_bits(p, 1); // video signal type = no clue
...
static int __inline encode_vol_header(BW* p, M4V_VOL* vol)
{
	...
	put_zero_bits(p, 1); // random_accessible_vol
	put_bits(p, 8, 1); // video_object_type_indication = Simple Object Type
	
	put_zero_bits(p, 1); //is_object_layer_identifier

	put_bits(p, 4, 1); // *** aspect_ratio_info = 1(1:1)
	
	put_zero_bits(p, 1); //vol_control_parameters
	
	put_zero_bits(p, 2); // shape_type
	put_bits(p, 1, 1); // marker
	...
	put_bits(p, 1, 1); // marker
	put_zero_bits(p, 1); // fixed vop rate = no
	put_bits(p, 1, 1); // marker
	put_bits(p, 13, vol->width); // width
	put_bits(p, 1, 1); // marker
	put_bits(p, 13, vol->height); // height
	put_bits(p, 1, 1); // marker
	put_zero_bits(p, 1); // progressive = false
...
*/
	...
	// !!!!! usually small number, no need to optimize here.
	while (time_incr--)
		put_bits(p, 1, 1);
	...
	put_zero_bits(p, 3); // intra dc VLC threashold
...
#ifdef C_OPT_MG
		M4V_BLOCK * blk = &mb->block[0];
		blk->block[0] = dcpred_for_enc(dcpred, 0, blk->block[0]);
		blk++;
		blk->block[0] = dcpred_for_enc(dcpred, 1, blk->block[0]);
		blk++;
		blk->block[0] = dcpred_for_enc(dcpred, 2, blk->block[0]);
		blk++;
		blk->block[0] = dcpred_for_enc(dcpred, 3, blk->block[0]);
		blk++;
		blk->block[0] = dcpred_for_enc(dcpred, 4, blk->block[0]);
		blk++;
		blk->block[0] = dcpred_for_enc(dcpred, 5, blk->block[0]);
#else
		for (n = 0; n < 6; n ++)
		{
