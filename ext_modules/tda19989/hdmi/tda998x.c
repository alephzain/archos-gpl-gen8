...
#ifdef USER_SET_INPUT_OUTPUT
static int mHdcpLoaded = 0;
#endif

#ifdef ANDROID_DSS
static int mDisplayOn = 0;
#endif
...
void hdcp_onoff(tda_instance *this) {
   ...
   if (this->tda.hdcp_status != HDCP_IS_NOT_INSTALLED) { /* check HDCP is installed ... */
      if (this->tda.hdcp_status == TMDL_HDMITX_HDCP_CHECK_NOT_STARTED) { /* ... and not started */
      ...
      else { /* HDCP is installed and started...  */
...
static void eventCallbackTx(tmdlHdmiTxEvent_t event)
{
   ...
   case TMDL_HDMITX_RX_DEVICE_ACTIVE: /* TV is ready to receive */
      ...
      if (mDisplayOn == 1) show_video(this);
...
static int hdmi_tx_init(tda_instance *this)
{
   ...
   /*    this->tda.setio.audio_in.i2sFormat = TMDL_HDMITX_I2SFOR_OTH_L; */
   /*    this->tda.setio.audio_in.i2sQualifier = TMDL_HDMITX_I2SQ_OTHERS; */
...
#ifdef USER_SET_INPUT_OUTPUT
    if (mHdcpLoaded == 1)  this->tda.hdcp_enable = 1;
#endif
...
{
   printk("hdmi_panel_disable called\n");
   hdmi_disable(1);
...
static int this_cdev_ioctl(struct inode *pInode, struct file *pFile, unsigned int cmd, unsigned long arg)
{
   ...
   switch ( _IOC_NR(cmd) )
      ...
      case TDA_SET_POWER_CMD:
         {
            tda_power power;
            BUG_ON(copy_from_user(&power,(tda_power*)arg,sizeof(tda_power)) != 0);
            if (mDisplayOn != 1) {
                /* If we are on the hdmi panel, do not switch the power through the ioctl, this will be done when disabling*/
                this->tda.power = power;
...
static int hdcpstate_save;

static int this_i2c_suspend(struct i2c_client *client, pm_message_t mesg)
{
        struct archos_hdmi_platform_data *pdata = client->dev.platform_data;
        tda_instance *this=&our_instance;
	
        //The I2C goes in suspend so hdmi module can go in suspend too
	tmdlHdmiTxSetPowerState(this->tda.instance,tmPowerStandby);
	/* update HPD */
	tmdlHdmiTxGetHPDStatus(this->tda.instance,&this->tda.hot_plug_detect);

	if (pdata->hdmi_pwr)
		pdata->hdmi_pwr(client, 0);
	
	/* add a strategic 20ms delay here to allow I2C to suspend */
	mdelay(20);
	
	return 0;
}

static int this_i2c_resume(struct i2c_client *client)
{
        struct archos_hdmi_platform_data *pdata = client->dev.platform_data;
	tda_instance *this=&our_instance;

	if (pdata->hdmi_pwr)
		pdata->hdmi_pwr(client, 1);
	
	//The I2C is resuming so hdmi module can also resume to previous power state
	tmdlHdmiTxSetPowerState(this->tda.instance,this->tda.power);
	/* update HPD */
	tmdlHdmiTxGetHPDStatus(this->tda.instance,&this->tda.hot_plug_detect);
...
static struct i2c_driver this_i2c_driver = {
   ...
   .suspend = this_i2c_suspend,
   .resume = this_i2c_resume,
                          ...
                          struct device_attribute *attr, const char *buf, size_t size)
{
   ...
   for(i=0;i<=strlen(buf);i++) { /* FIXME : use scanf instead */
      if (buf[i] >= '0' && buf[i] <= '9')
         resolution = resolution * 10 + buf[i] -'0'; 	   
      else 
         printk("wrong format in %s\n",__func__);
      break;
   }
    
   LOG(KERN_INFO,"format changed from %d to %d\n",this->tda.setio.video_in.format,resolution);
   this->tda.setio.video_out.format = resolution;
   this->tda.setio.video_in.format = resolution;
