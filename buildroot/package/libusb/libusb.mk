...
ifneq ($(LIBUSB_PATCH_FILE),)
	...
	toolchain/patch-kernel.sh $(LIBUSB_DIR) package/libusb/ *.patch
...
$(STAGING_DIR)/lib/libusb.so: $(LIBUSB_DIR)/.configured
	$(MAKE) -C $(LIBUSB_DIR)
	$(MAKE) prefix=$(STAGING_DIR)/usr -C $(LIBUSB_DIR) install
...
libusb-clean:
	rm -f $(TARGET_DIR)/usr/lib/libusb*.so*
	$(MAKE) prefix=$(STAGING_DIR)/usr -C $(LIBUSB_DIR) uninstall
