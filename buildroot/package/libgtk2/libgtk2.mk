...
LIBGTK2_BUILD_ENV= \
		...
		CPPFLAGS="$(CPPFLAGS) -I$(STAGING_DIR)/usr/include/cairo -I$(STAGING_DIR)/include/atk-1.0/ -I$(STAGING_DIR)/usr/include/pango-1.0/ -I$(STAGING_DIR)/usr/include/glib-2.0/"\
...
$(STAGING_DIR)/usr/lib/$(LIBGTK2_BINARY): $(LIBGTK2_DIR)/gtk/.libs/$(LIBGTK2_BINARY)
	$(MAKE) DESTDIR=$(STAGING_DIR) -C $(LIBGTK2_DIR) install
	touch -c $(STAGING_DIR)/usr/lib/$(LIBGTK2_BINARY)

$(TARGET_DIR)/usr/lib/libgtk-x11-2.0.so.0: $(STAGING_DIR)/usr/lib/$(LIBGTK2_BINARY)
	cp -a $(STAGING_DIR)/usr/lib/libgtk-x11-2.0.so $(TARGET_DIR)/usr/lib/
	cp -a $(STAGING_DIR)/usr/lib/libgtk-x11-2.0.so.0* $(TARGET_DIR)/usr/lib/
	cp -a $(STAGING_DIR)/usr/lib/libgdk*-2.0.so $(TARGET_DIR)/usr/lib/
	cp -a $(STAGING_DIR)/usr/lib/libgdk*-2.0.so.0* $(TARGET_DIR)/usr/lib/
	$(STRIPCMD) $(STRIP_STRIP_UNNEEDED) $(TARGET_DIR)/usr/lib/libgtk-x11-2.0.so.0*
	$(STRIPCMD) $(STRIP_STRIP_UNNEEDED) $(TARGET_DIR)/usr/lib/libgdk*-2.0.so.0*
	mkdir -p $(TARGET_DIR)/usr/lib/gtk-2.0/2.10.0/engines
	cp -a $(STAGING_DIR)/usr/lib/gtk-2.0/2.10.0/engines/*.so \
		$(TARGET_DIR)/usr/lib/gtk-2.0/2.10.0/engines/
	mkdir -p $(TARGET_DIR)/usr/lib/gtk-2.0/2.10.0/printbackends
	cp -a $(STAGING_DIR)/usr/lib/gtk-2.0/2.10.0/printbackends/*.so \
		$(TARGET_DIR)/usr/lib/gtk-2.0/2.10.0/printbackends/
	mkdir -p $(TARGET_DIR)/usr/lib/gtk-2.0/2.10.0/immodules
	cp -a $(STAGING_DIR)/usr/lib/gtk-2.0/2.10.0/immodules/*.so \
		$(TARGET_DIR)/usr/lib/gtk-2.0/2.10.0/immodules/
	mkdir -p $(TARGET_DIR)/usr/lib/gtk-2.0/2.10.0/loaders
	cp -a $(STAGING_DIR)/usr/lib/gtk-2.0/2.10.0/loaders/*.so \
		$(TARGET_DIR)/usr/lib/gtk-2.0/2.10.0/loaders/
...
libgtk2: uclibc png jpeg tiff $(XSERVER) libglib2 \
	cairo pango atk $(TARGET_DIR)/usr/lib/libgtk-x11-2.0.so.0 xorg-lib-Xext xorg-lib-Xcursor xorg-lib-Xinerama xorg-lib-Xfixes xorg-lib-Xrandr xorg-lib-Xrender xorg-lib-Xi
