/****************************************************************************
** $Id: qkeyboard_qws.cpp 8487 2006-06-26 09:45:02Z frank $
**
...
#define QT_TTY_KEYBOARD_PIPE_FILTERED	"/tmp/kbd_filtered"
...
{
    terminalName = QT_TTY_KEYBOARD_PIPE_FILTERED;
    if ((kbdFD = ::open(terminalName.local8Bit().data(), O_RDONLY | O_NDELAY)) < 0) {
	qDebug( "Cannot open %s (%s) -> fallback", terminalName.latin1(), strerror(errno));

	terminalName = device.isEmpty() ? "/dev/tty0" : device.latin1();
	if (kbdFD = ::open(terminalName, O_RDONLY | O_NDELAY, 0)) {
	    qDebug( "Cannot open %s (%s)", terminalName.latin1(), strerror(errno));
	}
    }
    ...
    if ( kbdFD >= 0 && terminalName.compare(QT_TTY_KEYBOARD_PIPE_FILTERED) != 0) {
	// save for restore.
...
QWSVFbKeyboardHandler::QWSVFbKeyboardHandler()
{
    ...
    terminalName = QT_VFB_KEYBOARD_PIPE_FILTERED;
