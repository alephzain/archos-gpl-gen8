...
/*
 * Configure the archos's bluetooth chip to use SCO over PCM
 */
int
sco_over_pcm_send(int index)
{
	int hcifd;
	struct hci_request rq;
	uint8_t params_config[34] = {0x00, 0x08, 0x00, 0x40, 0x1F, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x10, 0x00, 0x01, 0x00, 0x00, 0x10, 0x00, 0x01, 0x00, 0x01, 0x00, 0x10, 0x00, 0x11, 0x00, 0x00, 0x10, 0x00, 0x11, 0x00, 0x01, 0x00};
	uint8_t params_config_enhanced[26] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x04, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x04, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
	uint8_t params[2] = {0}, result[1024] = {0};

	info("Configuring SCO over PCM");

	if ((hcifd = hci_open_dev(index)) < 0) {
		error("Cannot open HCI socket");
		return -1;
	}

	/* Send_HCI_VS_Write_CODEC_Config */
	memset(&rq, 0, sizeof(rq));
	rq.ogf    = 0x3f;	/* vendor specific (VF) */
	rq.ocf    = 0x106;	/* HCI_VS_Write_CODEC_Config */
	rq.cparam = params_config;
	rq.clen   = 34;		/* params len */
	rq.event  = EVT_CMD_COMPLETE;
	rq.rparam = result;
	rq.rlen   = sizeof(result);
	if (hci_send_req(hcifd, &rq, SEND_TIMEOUT) < 0) {
		error("%s: Cannot send HCI command (0x%x 0x%x)", __FUNCTION__, rq.ogf, rq.ocf);
		goto err;
	}
	if (result[0] != 0) {
		error("%s: HCI command (0x%x 0x%x) failed", __FUNCTION__, rq.ogf, rq.ocf);
		goto err;
	}

	/* Send_HCI_VS_Write_CODEC_Config_Enhanced */
	memset(&rq, 0, sizeof(rq));
	rq.ogf    = 0x3f;	/* vendor specific (VF) */
	rq.ocf    = 0x107;	/* HCI_VS_Write_CODEC_Config_Enhanced */
	rq.cparam = params_config_enhanced;
	rq.clen   = 26;		/* params len */
	rq.event  = EVT_CMD_COMPLETE;
	rq.rparam = result;
	rq.rlen   = sizeof(result);
	if (hci_send_req(hcifd, &rq, SEND_TIMEOUT) < 0) {
	...
	/* Send_HCI_Write_SCO_Flow_Control_Enable */
	memset(&rq, 0, sizeof(rq));
	rq.ogf    = 0x03;	/* Host Controller and Baseband */
	rq.ocf    = 0x02f;	/* Send_HCI_Write_SCO_Flow_Control_Enable */
	params[0] = 0x1;	/* enable */
	rq.cparam = params;
	rq.clen   = 1;		/* params len */
	rq.event  = EVT_CMD_COMPLETE;
	rq.rparam = result;
	rq.rlen   = sizeof(result);
	if (hci_send_req(hcifd, &rq, SEND_TIMEOUT) < 0) {
...
/*
 * Configure the archos's bluetooth chip to use SCO over HCI
 */
int
sco_over_hci_send(int index)
{
	int hcifd;
	struct hci_request rq;
	uint8_t params[5] = {0}, result[1024] = {0};

	info("Configuring SCO over HCI\n");
	...
	memset(&rq, 0, sizeof(rq));
	rq.ogf    = 0x3f;	/* vendor specific (VF) */
	rq.ocf    = 0x210;	/* HCI_VS_Write_SCO_Configuration */
	params[0] = 0x1;	/* use host connection (HCI) */
	params[1] = 0x78;	/* packet size in bytes */
	params[3] = 0x01;	/* maximum latency in bytes */
	params[2] = 0xff;	/*     511 bytes */
	params[4] = 0xff;	/* do not change acceptation of packets with bad CRC */
	rq.cparam = params;
	rq.clen   = 5;		/* params len */
	rq.event  = EVT_CMD_COMPLETE;
	rq.rparam = result;
	rq.rlen   = sizeof(result);
	if (hci_send_req(hcifd, &rq, SEND_TIMEOUT) < 0) {
	...
	/* Send_HCI_Write_SCO_Flow_Control_Enable 0x01 */
	memset(&rq, 0, sizeof(rq));
	rq.ogf    = 0x03;	/* Host Controller and Baseband */
	rq.ocf    = 0x02f;	/* Send_HCI_Write_SCO_Flow_Control_Enable */
	params[0] = 0x1;	/* enable */
...
static void device_devup_setup(int index)
{
	...
	//sco_over_hci_send(index);
	sco_over_pcm_send(index);
